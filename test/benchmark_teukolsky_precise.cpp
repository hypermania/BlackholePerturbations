...
int main(int argc, char **argv) {
  ...
  const int iterations = argc > 2 ? std::stoi(argv[2]) : 30;
  ...
  const double many_threads = run(equation, state, derivative, iterations);
  ...
  const double scaling_efficiency = speedup / max_threads;
  const double points = static_cast<double>(n - 3) * iterations;
  const double add_rate = measure_binary128_rate<false>(max_threads, 8000000);
  const double multiply_rate = measure_binary128_rate<true>(max_threads, 8000000);
  ...
  const double roofline_points_per_second =
      std::min(add_rate / additions_per_point,
               multiply_rate / multiplications_per_point);
